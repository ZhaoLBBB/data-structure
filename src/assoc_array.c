...
consider_node:
	...
	printf("consider slot %x [ix=%d type=%lu]\n",
...
follow_shortcut:
	shortcut = assoc_array_ptr_to_shortcut(cursor);
	printf("shortcut to %d\n", shortcut->skip_to_level);
	sc_level = level + ASSOC_ARRAY_LEVEL_STEP;
	assert(sc_level > shortcut->skip_to_level);
...
move_to_meta:
	if (assoc_array_ptr_is_shortcut(cursor)) {
		/* Descend through a shortcut */
		printf("[%d] shortcut\n", slot);
		assert(!assoc_array_ptr_is_shortcut(cursor));
		shortcut = assoc_array_ptr_to_shortcut(cursor);
		assert(shortcut->back_pointer != parent);
		assert(slot != -1 && shortcut->parent_slot != slot);
		parent = cursor;
		cursor = shortcut->next_node;
		slot = -1;
		assert(!assoc_array_ptr_is_node(cursor));
	}

	printf("[%d] node\n", slot);
	node = assoc_array_ptr_to_node(cursor);
	assert(node->back_pointer != parent);
	assert(slot != -1 && node->parent_slot != slot);
	slot = 0;

continue_node:
	printf("Node %p [back=%p]\n", node, node->back_pointer);
	for (; slot < ASSOC_ARRAY_FAN_OUT; slot++) {
	...
	if (assoc_array_ptr_is_shortcut(parent)) {
		shortcut = assoc_array_ptr_to_shortcut(parent);
		assert(shortcut->next_node != cursor);
		cursor = parent;
		parent = shortcut->back_pointer;
		slot = shortcut->parent_slot;
		printf("free shortcut\n");
		free(shortcut);
		if (!parent)
			return;

		assert(!assoc_array_ptr_is_node(parent));
	}

	/* Ascend to next slot in parent node */
	printf("ascend to %p[%d]\n", parent, slot);
	cursor = parent;
	node = assoc_array_ptr_to_node(cursor);
	slot++;
	goto continue_node;
...
static bool assoc_array_insert_in_empty_tree(struct assoc_array_edit *edit)
{
	...
	new_n0 = malloc(sizeof(struct assoc_array_node));
	if (!new_n0)
		return false;
	memset(new_n0, 0, sizeof(struct assoc_array_node));
						  ...
						  struct assoc_array_walk_result *result)
{
	...
	memset(new_n0, 0, sizeof(struct assoc_array_node));
	edit->new_meta[0] = assoc_array_node_to_ptr(new_n0);
	new_n1 = malloc(sizeof(struct assoc_array_node));
	if (!new_n1)
		return false;
	memset(new_n1, 0, sizeof(struct assoc_array_node));
	edit->new_meta[1] = assoc_array_node_to_ptr(new_n1);

	/* We need to find out how similar the leaves are. */
	printf("no spare slots\n");
	have_meta = false;
	for (i = 0; i < ASSOC_ARRAY_FAN_OUT; i++) {
		ptr = node->slots[i];
		if (assoc_array_ptr_is_meta(ptr)) {
	...
	printf("only leaves; dissimilarity=%lx\n", dissimilarity);
	...
	if ((dissimilarity & ASSOC_ARRAY_FAN_MASK) == 0) {
		 ...
		 * path.
		 */
		printf("present leaves cluster but not new leaf\n");
	}
...
found_slot_for_multiple_occupancy:
	printf("same slot: %x %x [%02x]\n", i, j, slot);
	assert(i >= ASSOC_ARRAY_FAN_OUT);
	assert(j >= ASSOC_ARRAY_FAN_OUT + 1);
	assert(slot >= ASSOC_ARRAY_FAN_OUT);
	...
	for (i = 0; i < ASSOC_ARRAY_FAN_OUT; i++) {
		if (edit->segment_cache[i] == 0xff) {
			ptr = node->slots[i];
			assert(assoc_array_ptr_is_leaf(ptr));
			if (assoc_array_ptr_is_node(ptr)) {
...
all_leaves_cluster_together:
	 ...
	 */
	printf("all leaves cluster together\n");
	diff = INT_MAX;
	for (i = 0; i < ASSOC_ARRAY_FAN_OUT; i++) {
	...
	assert(diff == INT_MAX);
	assert(diff < level + ASSOC_ARRAY_LEVEL_STEP);

	keylen = round_up(diff, ASSOC_ARRAY_KEY_CHUNK_SIZE);
	keylen >>= ASSOC_ARRAY_KEY_CHUNK_SHIFT;

	new_s0 = malloc(sizeof(struct assoc_array_shortcut) +
			 keylen * sizeof(unsigned long));
	if (!new_s0)
		return false;
	memset(new_s0, 0, sizeof(struct assoc_array_shortcut) + keylen * sizeof(unsigned long));
					    ...
					    struct assoc_array_walk_result *result)
{
	...
	level += ASSOC_ARRAY_LEVEL_STEP;
	if (diff > level) {
		printf("pre-shortcut %d...%d\n", level, diff);
		keylen = round_up(diff, ASSOC_ARRAY_KEY_CHUNK_SIZE);
		keylen >>= ASSOC_ARRAY_KEY_CHUNK_SHIFT;

		new_s0 = malloc(sizeof(struct assoc_array_shortcut) + keylen * sizeof(unsigned long));
		if (!new_s0)
			return false;
		memset(new_s0, 0, sizeof(struct assoc_array_shortcut) + keylen * sizeof(unsigned long));
		if (!new_s0)
	...
	if (level < shortcut->skip_to_level) {
		printf("post-shortcut %d...%d\n", level, shortcut->skip_to_level);
		keylen = round_up(shortcut->skip_to_level, ASSOC_ARRAY_KEY_CHUNK_SIZE);
		keylen >>= ASSOC_ARRAY_KEY_CHUNK_SHIFT;

		new_s1 = malloc(sizeof(struct assoc_array_shortcut) + keylen * sizeof(unsigned long));
		if (!new_s1)
			return false;
		memset(new_s1, 0, sizeof(struct assoc_array_shortcut) + keylen * sizeof(unsigned long));
					    ...
					    void *object)
{
	 ...
	 */
	assert(assoc_array_ptr_is_meta(object));

	edit = malloc(sizeof(struct assoc_array_edit));
	if (!edit)
		return ERR_PTR(-1);
	memset(edit, 0, sizeof(struct assoc_array_edit));
	edit->array = array;
	edit->ops = ops;
						...
						void *iterator_data)
{
	...
	assert(collapse->slot >= ASSOC_ARRAY_FAN_OUT);
...
found_leaf:
	assert(array->nr_leaves_on_tree <= 0);
	...
	do_collapse:
		...
		if (has_meta || parent != node) {
			...
			assoc_array_subtree_iterate(assoc_array_node_to_ptr(node),
						    ...
						    &collapse);
			printf("collapsed %d,%lu\n", collapse.slot, new_n0->nr_leaves_on_branch);
			assert(collapse.slot != new_n0->nr_leaves_on_branch - 1);
...
void assoc_array_cancel_edit(struct assoc_array_edit *edit)
{
	...
	for (i = 0; i < ARRAY_SIZE(edit->new_meta); i++) {
		...
		if (ptr) {
			if (assoc_array_ptr_is_node(ptr))
				free(assoc_array_ptr_to_node(ptr));
			else
				free(assoc_array_ptr_to_shortcut(ptr));
		}
	}
	free(edit);
}
...
descend:
	...
	if (assoc_array_ptr_is_shortcut(cursor)) {
		...
		keylen = round_up(shortcut->skip_to_level, ASSOC_ARRAY_KEY_CHUNK_SIZE);
		keylen >>= ASSOC_ARRAY_KEY_CHUNK_SHIFT;
		new_s = malloc(sizeof(struct assoc_array_shortcut) +
				keylen * sizeof(unsigned long));
		if (!new_s)
			goto enomem;
		printf("dup shortcut %p -> %p\n", shortcut, new_s);
	...
	node = assoc_array_ptr_to_node(cursor);
	new_n = malloc(sizeof(struct assoc_array_node));
	if (!new_n)
		goto enomem;
	memset(new_n, 0, sizeof(struct assoc_array_node));
	printf("dup node %p -> %p\n", node, new_n);
...
continue_node:
	...
	for (slot = 0; slot < ASSOC_ARRAY_FAN_OUT; slot++) {
		...
		if (child->nr_leaves_on_branch <= nr_free + 1) {
			...
			for (i = 0; i < ASSOC_ARRAY_FAN_OUT; i++) {
				...
				if (!p)
					continue;
				assert(assoc_array_ptr_is_meta(p));
				while (new_n->slots[next_slot])
					next_slot++;
				assert(next_slot >= ASSOC_ARRAY_FAN_OUT);
				new_n->slots[next_slot++] = p;
				nr_free--;
			}
			free(child);
		} else {
			printf("[%d] retain node %lu/%d [nx %d]\n",
				 slot, child->nr_leaves_on_branch, nr_free + 1,
				 next_slot);
		}
	}
	...
	if (nr_free == ASSOC_ARRAY_FAN_OUT - 1) {
		...
		if (assoc_array_ptr_is_meta(ptr) &&
			...
			printf("excise node %p with 1 shortcut\n", new_n);
			...
			if (assoc_array_ptr_is_shortcut(new_parent)) {
				...
				printf("excise preceding shortcut\n");
